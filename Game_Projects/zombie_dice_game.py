...
def fillTheCup():
    #Filling the cup with 13 shuffled dice
    cup = []
...
def drawDice(cup, count):
    #Draw random dice from a cup
    diceDrawn = []
    if len(cup) < 3:
        count = len(cup)
...
def whoGoesFirst():
    #Decides who's turn is first
    if random.randint(0, 1) == 0:
        return 'Computer'
    else:
        return 'Player'

def orderDice (forRedrawList, redrawnDiceList, diceLeft):
        ...
        if turn == 'Player':
            ...
            if answer.lower().startswith('y') or len(answer) == 0:
                ...
                while len(diceLeft) >= 0:
                    ...
                    if len(playerSides) == 3 and playerSides[0] == 'shotgun' and playerSides[1] == 'shotgun' and playerSides[2] == 'shotgun':
                        turn = 'Computer'
                        playerTurnPoints = 0
                        ...
                        print(f'After this draw you could add {playerTurnPoints} point(s) to your total score or keep rolling the dice.')
                        ...
                        if playerRedraw.lower().startswith('y') or len(playerRedraw) == 0:
                            ...
                            print(f'You got {playerStepsCount} footsteps, so you are taking {3 - playerStepsCount} dice from the cup to rerall. Press Enter if you are ready')
                            input()
            ...
            while len(diceLeft) >= 0:
                ...
                if len(computerSides) == 3 and computerSides[0] == 'shotgun' and computerSides[1] == 'shotgun' and computerSides[2] == 'shotgun':
                    turn = 'Player'
                    computerTurnPoints = 0
